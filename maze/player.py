...
class Player(pygame.sprite.Sprite):
    def __init__(self, center_x, center_y, forward_angle):
        ...
        self.width =100
        self.height = 50
        self.forward_angle = forward_angle  # 在游戏坐标系中，顺时针旋转的角度
    ...
    def update_delta_time(self):
        ...
        self.delta_time = (cur_time - self.last_time) / 1000  # 将单位转换成秒
    ...
    def move(self, direction=1):
        if direction == 1 and abs(self.move_velocity) > 50:
            self.rotate(direction)  # 只有车在前进或后退时，车身角度才会变化；原地不动打方向盘，车身角度是不变的
